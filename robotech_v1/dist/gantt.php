...
if (!$proyecto_id || !is_numeric($proyecto_id)) {
    die("⚠️ Proyecto no especificado correctamente.");
}
...
$hoy = date('Y-m-d');
$responsables = $conn->query("SELECT DISTINCT responsable FROM tareas WHERE proyecto_id = $proyecto_id AND responsable IS NOT NULL AND responsable != ''")->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
...
<main class="lg:ms-[260px] pt-[90px] px-4 pb-16">
    <div class="max-w-6xl mx-auto bg-white p-6 rounded shadow">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">📊 Gantt del Proyecto #<?= $proyecto_id ?></h1>
            <a href="tareas.php?proyecto_id=<?= $proyecto_id ?>" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">🔄 Ver como tabla</a>
        </div>

        <!-- 🔧 Filtros y controles -->
        <div class="controls flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
            <div class="flex gap-2">
                <button onclick="gantt.change_view_mode('Day')" class="border border-gray-300 px-3 py-2 rounded hover:bg-gray-100">📅 Día</button>
                <button onclick="gantt.change_view_mode('Week')" class="border border-gray-300 px-3 py-2 rounded hover:bg-gray-100">🗓 Semana</button>
                <button onclick="gantt.change_view_mode('Month')" class="border border-gray-300 px-3 py-2 rounded hover:bg-gray-100">📆 Mes</button>
            </div>

            <div class="flex gap-2">
                <select id="filtro-estado" class="border border-gray-300 px-3 py-2 rounded">
                ...
                <select id="filtro-responsable" class="border border-gray-300 px-3 py-2 rounded">
        ...
        <!-- 📈 Gantt render -->
        <div id="gantt" class="overflow-x-auto" style="min-height: 500px;"></div>
