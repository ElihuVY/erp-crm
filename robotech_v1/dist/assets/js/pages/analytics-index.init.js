...
fetch('facturas-data.php')
  .then(response => {
    if (!response.ok) throw new Error('Error en la respuesta: ' + response.status);
    return response.json();
  })
  .then(data => {
    // Validar que data tenga las propiedades que esperamos
    if (!data.lastYear || !data.thisYear) {
  ...
  .then(data => {
    ...
    // Extraer etiquetas (nombres) y datos (número de facturas)
    const labels = data.map(item => item.serie);
    const series = data.map(item => item.facturas);
    ...
    const options = {
      ...
      series: series,
      labels: labels,
      colors: colors.slice(0, labels.length), // limitar a cantidad de series
      legend: {
        show: false,
        position: 'bottom',
        horizontalAlign: 'center',
    ...
    chart.render();
    // Mostrar leyenda con nombre de series y sus colores
    const legendContainer = document.getElementById("series_legend");
    ...
    labels.forEach((label, index) => {
      ...
      li.innerHTML = `<i class="icofont-ui-play me-2" style="color:${color}"></i> ${label}`;
