...
// Crear factura automáticamente si el estado es 'aceptado'

// También verificar si ya existen presupuestos aceptados sin factura
...
if (isset($_POST['actualizar_estado']) && $_POST['estado'] === 'aceptado') {
    ...
    $res = $conn->query("SELECT * FROM presupuestos WHERE id = $presupuesto_id");
    ...
    if ($presupuesto && $presupuesto['convertido'] == 0) {
        ...
        $conn->query("UPDATE presupuestos SET convertido = 1, factura_id = $factura_id, estado = 'aceptado' WHERE id = $presupuesto_id");
    }
}
...
// Presupuestos
$res_presupuestos = $conn->query("SELECT * FROM presupuestos WHERE cliente_id = $cliente_id");

// Facturas
$res_facturas = $conn->query("SELECT * FROM facturas WHERE cliente_id = $cliente_id");
          ...
          <div class="w-full">
            <main class="lg:ms-[260px] pt-[90px] px-4 pb-16">
              <div class="max-w-5xl mx-auto bg-white p-6 rounded shadow">
                <h1 class="text-2xl font-bold mb-4">📄 Ficha de Cliente</h1>
                <a href="clientes.php" class="text-blue-600 hover:underline mb-4 inline-block">⬅️ Volver al listado</a>
                ...
                <div class="mb-6">
                  ...
                  <p><strong>Notas:</strong> <?php echo nl2br(htmlspecialchars($cliente['notas'])); ?></p>
                ...
                <div class="mb-6">
                  <h2 class="text-xl font-semibold">📄 Presupuestos Asociados</h2>
                  <?php if ($res_presupuestos->num_rows > 0): ?>
                    <ul class="list-disc ml-6">
                      <?php while ($p = $res_presupuestos->fetch_assoc()): ?>
                        <li>
                          <strong>#<?php echo $p['id']; ?></strong> - <?php echo $p['estado']; ?> - <?php echo $p['fecha_creacion']; ?> - <?php echo $p['importe']; ?> €
                          <?php if ($p['convertido']): ?>✅ Factura #<?php echo $p['factura_id']; ?><?php endif; ?>
                          <form method="POST" class="inline ml-4">
                            <input type="hidden" name="presupuesto_id" value="<?php echo $p['id']; ?>">
                            <select name="estado" class="border rounded p-1">
                              <option value="borrador" <?php if ($p['estado'] === 'borrador') echo 'selected'; ?>>Borrador</option>
                              <option value="enviado" <?php if ($p['estado'] === 'enviado') echo 'selected'; ?>>Enviado</option>
                              <option value="aceptado" <?php if ($p['estado'] === 'aceptado') echo 'selected'; ?>>Aceptado</option>
                            </select>
                            <button type="submit" name="actualizar_estado" class="text-sm bg-blue-500 text-white px-2 py-1 rounded">Actualizar</button>
                          </form>
                        </li>
                      <?php endwhile; ?>
                    </ul>
                  <?php else: ?>
                    <p class="text-gray-600">No hay presupuestos asociados.</p>
                  <?php endif; ?>
                </div>
                  ...
                  <?php if ($res_facturas->num_rows > 0): ?>
                    <ul class="list-disc ml-6">
                      <?php while ($f = $res_facturas->fetch_assoc()): ?>
                        <li>#<?php echo $f['id']; ?> - <?php echo $f['tipo']; ?> - <?php echo $f['estado']; ?> - <?php echo $f['fecha_emision']; ?> - <?php echo $f['total']; ?> €</li>
                      <?php endwhile; ?>
                    </ul>
                  <?php else: ?>
                    <p class="text-gray-600">No hay facturas asociadas.</p>
                  <?php endif; ?>
                </div>

                <div>
                  <h2 class="text-xl font-semibold">📁 Proyectos en Curso</h2>
                  <?php if ($res_proyectos && $res_proyectos->num_rows > 0): ?>
                    <ul class="list-disc ml-6">
                      <?php while ($pr = $res_proyectos->fetch_assoc()): ?>
                        <li>#<?php echo $pr['id']; ?> - <?php echo $pr['nombre']; ?> (<?php echo $pr['estado']; ?>)</li>
                      <?php endwhile; ?>
                    </ul>
                  <?php else: ?>
                    <p class="text-gray-600">No hay proyectos en curso.</p>
                  <?php endif; ?>
                </div>
              </div>
            </main>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function buscarClientes() {
      ...
      const params = new URLSearchParams({ buscar, empresa, solo_favoritos: soloFavoritos });
