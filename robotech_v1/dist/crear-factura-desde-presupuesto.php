...
// Si se selecciona un presupuesto
if (isset($_GET['presupuesto_id'])) {
    ...
    $check = $conn->query("SELECT id FROM facturas WHERE presupuesto_id = $presupuesto_id");
    if ($check->num_rows > 0) {
        $mensaje = "⚠ Ya se ha generado una factura para este presupuesto.";
    } else {
        // Obtener datos del presupuesto
        $presupuesto = $conn->query("SELECT * FROM presupuestos WHERE id = $presupuesto_id")->fetch_assoc();
        ...
        if ($presupuesto) {
            ...
            $stmt = $conn->prepare("INSERT INTO facturas (cliente_id, presupuesto_id, tipo, estado, fecha_emision, fecha_vencimiento, total)
                                    VALUES (?, ?, 'unica', 'emitida', ?, ?, ?)");
            $stmt->bind_param("iissd", $cliente_id, $presupuesto_id, $fecha_emision, $fecha_vencimiento, $total);
            $stmt->execute();

            // Marcar presupuesto como convertido (opcional)
            $conn->query("UPDATE presupuestos SET convertido = 1 WHERE id = $presupuesto_id");
          ...
          <div class="w-full">
            <main class="lg:ms-[260px] pt-[90px] px-4 pb-16">
              <div class="max-w-5xl mx-auto bg-white p-6 rounded shadow">
                <h2 class="text-2xl font-bold mb-6">🧾 Crear factura desde presupuesto</h2>
                ...
                <table class="w-full border border-collapse">
                  ...
                  <tbody>
                    <?php
                    $query = "SELECT p.*, c.nombre AS cliente FROM presupuestos p
                    ...
                    if ($result->num_rows === 0) {
                      echo "<tr><td colspan='5' class='p-4 text-center text-gray-500'>No hay presupuestos aceptados disponibles.</td></tr>";
                    }
                    ...
                    while ($row = $result->fetch_assoc()) {
                      echo "<tr>
                              ...
                              <td class='p-2 border'>
                                <a href='crear-factura-desde-presupuesto.php?presupuesto_id={$row['id']}' class='text-green-600 hover:underline'>Crear factura</a>
                              </td>
                            </tr>";
                    }
                    ?>
